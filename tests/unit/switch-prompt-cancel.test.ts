...
let promptImpl: (...args: unknown[]) => Promise<unknown> = async () => ({
  selectedWorkspace: 'task-one',
});

mock.module('inquirer', () => ({
  default: {
    prompt: (...args: unknown[]) => promptImpl(...args),
  },
}));
...
test('switch prompt cancel exits 0 without error output', async () => {
